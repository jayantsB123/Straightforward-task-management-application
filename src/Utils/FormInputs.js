...
const FormInputs = ({
  ...
  const handleSelected = (catagoryObj) => {
    const isCategorySelected = selectedCatagory.find(
      (val) => val.id === catagoryObj.id
    );
    if (isCategorySelected) {
      setSelectedCatagory([]);
    } else {
      setSelectedCatagory([catagoryObj]);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="max-w-[600px] m-auto">
          ...
          {CategoryOpen ? (
            <div className="mt-3">
              <ul className=" p-2 bg-purple-400 flex flex-col gap-2 max-sm:gap-1 rounded-xl">
                <li className=" my-2 px-3 text-white max-sm:text-sm">Select(atmost 1)</li>
        ...
        <div className="text-center mt-4">
          <button
            ...
            } transition text-xl font-bold bg-indigo-900 p-4 max-sm:p-3 max-sm:text-lg rounded-xl w-full`}
          >
            Create Task
          </button>
        </div>
